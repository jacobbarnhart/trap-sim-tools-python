...
def exact_saddle(V, X, Y, Z, dim, scale = 1, Z0 = None):
    ...
    #################################################################################################    
    if dim == 2: 
        # Finds the index of the center (ie Z0) of Z, which is evenly spaced negative values -> 0 -> positive values.
        if len(V.shape) == 3:
        ...
        # Returns the saddle point.
        I, J, K0 = find_saddle(V, X, Y, Z, 2, Z0 = Z0) 
        r0 = X[I], Y[J]
        # How are these numbers determined?
        ## Replace with constants or numerically determined variables.
        if (I < 2 or I > V.shape[0] - 2): 
            print('exact_saddle.py: Saddle point out of bounds in radial direction.\n')
            return r0
        if (J < 2 or J > V.shape[1] - 2):
...
def find_saddle(V, X, Y, Z, dim, scale = 1, Z0 = None):
    ...
    if dim == 2: # Extrapolate to the values of A at z0.
        ...
        if len(V.shape) == 3:
            ...
            # Again, just obtains the index in which Z is Z0.
            ## also unnecessary. method for this.
            for i in range(len(Z)):
                if Z[i - 1] < Z0 and Z[i] >= Z0:
                    Ks = i - 1
                    if Z0 < 1:
            ...
            if Ks >= len(Z):
                return('The selected coordinate is at the end of range.')
            v1 = V[:, :, Ks] # Potential to the left.
            v2 = V[:, :, Ks + 1] # Potential to the right ## Check this and make sure it is actually splicing correctly.
            V2 = v1 + (v2 - v1) * (Z0 - Z[Ks]) / (Z[Ks + 1] - Z[Ks]) # Average of the potentials to the left and right.
        V2s = V2.shape
        if len(V2s)!= 2: ## Is this still needed?
            return('Problem with find_saddle.py dimensionalities. It is {}.'.format(V2s))
        # This is the average potential "normalized" to the max point potential.
        f = V2 / float(np.max(abs(V2)))
        # x[1] - x[0] is the step size for the coorresponding axis.
        # Is the derivative of f with respect to the axis, thus it is E since E = grad(V)
        [Ex, Ey] = np.gradient(f, abs(X[1] - X[0]), abs(Y[1] - Y[0]))
        # Average of the derivatives.
        E = np.sqrt(Ex ** 2 + Ey ** 2)
        # Min of the derivates. ## Pretty sure you don't have to cast to float?
        m = float(np.min(E))
        mr = E[0, 0]
        Is, Js = 1, 1 # in case there is no saddle
        ## This is unnecessary code. This just finds the index of the min.
        ...
        # This origin corresponds to the indices of the min of E and where Z = Z0
        origin = [Is, Js, Ks]
...
def sum_of_e_field_2d(r, z0, V, X, Y, Z, exact_saddle = True):
    ...
    second order multipole terms in matrix V, when the center of the multipoles
    is at x0, y0, z0.
    Used by exact_saddle for 3D saddle search.
    Note that order of outputs for spher_harm_exp are changed, but 1 to 3 should still be E field.
    '''
    ...
    s = c ** 2
    f = sum(s[1:4]) / sum(s[4:9])
    # Can f even have an imaginary part?
    real_f = np.real(f[0])
    return real_f
